...
int main(int argc, const char * argv[]) {
    ...
    cout << "married female: " << endl;
    cout << "\t" << cbk->fmal_marry.size() << endl;
    for(int i = 0; i < 11; ++i){
        cout << "\t" << cbk->fmal_cbrs[i].size() << endl;
    }
    ...
    for(int year = 0; year < 40; ++year){
        ...
        deaths = 0;
        cbk->sim_pop(year);
        cout << "year = " << year+2011 << " birth = " << births << " death = " << deaths  << " mgrt = " << int(cbk->cpop*cbk->pop_loss[year]/1000) << endl;
        cbk->get_pop(year);
        cbk->get_hhold(year);
        cout << "vant bldg = " << cbk->cblok_vcnt_rbldgs.size() << endl;
        
        if(year % 10 == 0) cbk->get_hhold_size(year);
    }
